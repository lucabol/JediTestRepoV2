...
- Always refer to the [source project wiki] for detailed configuration setup
- Always use the latest [source project releases] which includes the latest features and bug fixes
- Please use the [source project issues] section to report any issues that you encounter as well as any requested features that you would like to raise to the development team. Always remember to close your issues when its gets resolved 
- Remember to subscribe to the source repo (at least subscribe to be notified about new releases) to stay in the loop as we are always adding new features and squashing these pesky bugs
