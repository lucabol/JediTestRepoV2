...
internal static class ApiModule
{
    ...
    private static ExtractApis GetExtractApis(IServiceProvider provider)
    {
        ...
        return async cancellationToken =>
        {
            ...
            await list(cancellationToken)
                    // Group APIs by version set (source project issue reference).
    ...
    private static ListApis GetListApis(IServiceProvider provider)
    {
        ...
        bool shouldExtractApiDto(ApiDto dto) =>
            // When explicit API names are configured, skip the version-set filter —
            // the caller already narrowed the list to the requested APIs.
            findConfigurationApis().IsSome
