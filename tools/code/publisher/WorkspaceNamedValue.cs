...
internal static class WorkspaceNamedValueModule
{
    ...
    private static PutWorkspaceNamedValueInApim GetPutWorkspaceNamedValueInApim(IServiceProvider provider)
    {
        ...
        return async (name, dto, workspaceName, cancellationToken) =>
        {
            ...
            {
                logger.LogWarning(
                    "Named value {NamedValueName} in workspace {WorkspaceName} is secret but has no value or Key Vault reference — it cannot be provisioned by the publisher and will be skipped. " +
                    "If any backend in workspace {WorkspaceName} has credentials (header or query) that reference this named value, the backend PUT will fail with a 400 ValidationError from APIM. " +
                    ...
                    "or pre-create the named value manually in the target APIM workspace before running the publisher.",
