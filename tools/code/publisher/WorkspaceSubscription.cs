...
public delegate Option<(SubscriptionName Name, WorkspaceName WorkspaceName)> TryParseWorkspaceSubscriptionName(FileInfo file);
public delegate bool IsWorkspaceSubscriptionNameInSourceControl(SubscriptionName name, WorkspaceName workspaceName);
public delegate ValueTask PutWorkspaceSubscription(SubscriptionName name, WorkspaceName workspaceName, CancellationToken cancellationToken);
public delegate ValueTask<Option<WorkspaceSubscriptionDto>> FindWorkspaceSubscriptionDto(SubscriptionName name, WorkspaceName workspaceName, CancellationToken cancellationToken);
public delegate ValueTask PutWorkspaceSubscriptionInApim(SubscriptionName name, WorkspaceSubscriptionDto dto, WorkspaceName workspaceName, CancellationToken cancellationToken);
public delegate ValueTask DeleteWorkspaceSubscriptions(CancellationToken cancellationToken);
public delegate ValueTask DeleteWorkspaceSubscription(SubscriptionName name, WorkspaceName workspaceName, CancellationToken cancellationToken);
public delegate ValueTask DeleteWorkspaceSubscriptionFromApim(SubscriptionName name, WorkspaceName workspaceName, CancellationToken cancellationToken);
...
internal static class WorkspaceSubscriptionModule
{
    ...
    private static PutWorkspaceSubscriptions GetPutWorkspaceSubscriptions(IServiceProvider provider)
    {
        ...
        return async cancellationToken =>
        {
            ...
            await getPublisherFiles()
                    ...
                    .Where(subscription => isNameInSourceControl(subscription.Name, subscription.WorkspaceName))
                    .Distinct()
    ...
    private static IsWorkspaceSubscriptionNameInSourceControl GetIsWorkspaceSubscriptionNameInSourceControl(IServiceProvider provider)
    {
        ...
        bool doesInformationFileExist(SubscriptionName name, WorkspaceName workspaceName)
        {
    ...
    private static PutWorkspaceSubscription GetPutWorkspaceSubscription(IServiceProvider provider)
    {
        ...
        return async (name, workspaceName, cancellationToken) =>
        {
            using var _ = activitySource.StartActivity(nameof(PutWorkspaceSubscription))
                                       ?.AddTag("workspace_subscription.name", name)
    ...
    private static PutWorkspaceSubscriptionInApim GetPutWorkspaceSubscriptionInApim(IServiceProvider provider)
    {
        ...
        {
            logger.LogInformation("Putting subscription {SubscriptionName} in workspace {WorkspaceName}...", name, workspaceName);

            await WorkspaceSubscriptionUri.From(name, workspaceName, serviceUri)
    ...
    private static DeleteWorkspaceSubscriptions GetDeleteWorkspaceSubscriptions(IServiceProvider provider)
    {
        ...
        return async cancellationToken =>
        {
            ...
            await getPublisherFiles()
                    .Choose(tryParseName.Invoke)
                    .Where(subscription => isNameInSourceControl(subscription.Name, subscription.WorkspaceName) is false)
                    .Distinct()
    ...
    private static DeleteWorkspaceSubscriptionFromApim GetDeleteWorkspaceSubscriptionFromApim(IServiceProvider provider)
    {
        ...
        return async (name, workspaceName, cancellationToken) =>
        {
            logger.LogInformation("Deleting subscription {SubscriptionName} from workspace {WorkspaceName}...", name, workspaceName);
